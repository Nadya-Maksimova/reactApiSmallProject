import { useState} from 'react';


function RandomBeer(): JSX.Element {
  const [ name, setName] = useState('Beer')
const [image, setImage] = useState('data:image/jpeg;base64,')
const [description, setDescription] = useState('Beer is the oldest and most widely consumed type of alcoholic drink in the world, and the third most popular drink overall after potable water and tea!')
  

  const getBeer = (): void => {
    fetch('https://api.punkapi.com/v2/beers/random')
    .then((response) => response.json())
    .then((data)=>{
    setName(data[0].name)
    setImage(data[0].image_url)
    setDescription(data[0].description)
    })
  };

  return ( 
<div className='container sm'>
    <div className='card'style={{backgroundColor:'#FCF4F2'}} >
    <p className="card-title" style={{color:'red', fontSize:'xx-large', fontFamily:'fantasy'}}>{name}</p>
    <center>
  <img className="card-img-top" style={{ width: '100px', height:"auto" }} src={image} alt="..."/>
  </center>
  <div className='card-body'>
    <p className="card-text" style={{color:'black', fontSize:'20px'}}>{description}</p>
  </div>
  <div>
  <button style={{fontSize:'1rem'}} onClick={getBeer} type="button" className="btn btn-warning">Press me</button>
  <br></br>
  <br></br>
  </div>
  </div>
  </div>

  
  );
  }

export default RandomBeer;
